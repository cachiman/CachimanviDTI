...
class LightAttention(nn.Module):
    def __init__(self, cfg, output_dim=2, kernel_size=9, n_layers=4):
        ...
        for i in range(n_layers):
            self.abg_layers.append(
                TransformerConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=4,
                                concat=False, edge_dim=self.embed_dim)
            )
        ...
        self.abg_fc1 = torch.nn.Linear(self.embed_dim, 1024)
        self.abg_bn1 = nn.BatchNorm1d(1024)
        self.abg_fc2 = torch.nn.Linear(1024, self.output_dim)
        self.abg_bn2 = nn.BatchNorm1d(self.output_dim)
