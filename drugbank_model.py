...
class LightAttention(nn.Module):
    def __init__(self, cfg, output_dim=2, kernel_size=9, n_layers=6):
        ...
        self.abg_conv1 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                         dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.abg_conv2 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                         dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.abg_conv3 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                         dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.abg_conv4 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                         dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.abg_conv5 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                        dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.abg_conv6 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                        dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.bag_conv1 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                         dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.bag_conv2 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                         dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.bag_conv3 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                         dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.bag_conv4 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                         dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.bag_conv5 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                        dropout=self.dropout_rate, edge_dim=self.embed_dim)
        self.bag_conv6 = GATConv(in_channels=self.embed_dim, out_channels=self.embed_dim, heads=1,
                                        dropout=self.dropout_rate, edge_dim=self.embed_dim)
    ...
    def forward(self, batch, mask, **kwargs):
        ...
        # # target cnn(xiaorong)
        # # t_o = self.feature_convolution(t_1D)  # [batch_size, embeddings_dim, sequence_length]
        # # t_o = self.dropout(t_o)
        # # attention = self.attention_convolution(t_1D)
        # # attention = attention.masked_fill(mask[:, None, :] == False, -1e9)
        # # t_o2, _ = torch.max(t_o, dim=-1)
        # # t_o = self.linear(t_o2)
        ...
        atom_h = self.abg_conv1(node_hidden,edge_index,edge_hidden)
        ...
        cur_angle_hidden = cur_angle_hidden.to(device)
        cur_edge_hidden = self.embed_bag_node(cur_edge_hidden)
        edge_h = self.bag_conv2(cur_edge_hidden, bag_edge_index, cur_angle_hidden)
        edge_h = self.relu(edge_h)
        ...
        cur_angle_hidden = cur_angle_hidden.to(device)
        cur_edge_hidden = self.embed_bag_node(cur_edge_hidden)
        edge_h = self.bag_conv3(cur_edge_hidden, bag_edge_index, cur_angle_hidden)
        edge_h = self.relu(edge_h)

        #4
        atom_h = self.abg_conv4(atom_h, edge_index, edge_h)
        atom_h = self.relu(atom_h)
        edge_int_feat = edge_attr_list[:, 0:3].to('cpu').to(torch.int64)
        edge_float_feat = edge_attr_list[:, 3].to('cpu').to(torch.float32)
        cur_edge_hidden = self.init_bond_embedding(edge_int_feat)
        ...
        cur_angle_hidden = cur_angle_hidden.to(device)
        cur_edge_hidden = self.embed_bag_node(cur_edge_hidden)
        edge_h = self.bag_conv4(cur_edge_hidden, bag_edge_index, cur_angle_hidden)
        edge_h = self.relu(edge_h)

        #5
        atom_h = self.abg_conv5(atom_h, edge_index, edge_h)
        atom_h = self.relu(atom_h)
        edge_int_feat = edge_attr_list[:, 0:3].to('cpu').to(torch.int64)
        edge_float_feat = edge_attr_list[:, 3].to('cpu').to(torch.float32)
        cur_edge_hidden = self.init_bond_embedding(edge_int_feat)
        ...
        #6
        atom_h = self.abg_conv6(atom_h, edge_index, edge_h)
        atom_h = self.relu(atom_h)
        edge_int_feat = edge_attr_list[:, 0:3].to('cpu').to(torch.int64)
        edge_float_feat = edge_attr_list[:, 3].to('cpu').to(torch.float32)
        cur_edge_hidden = self.init_bond_embedding(edge_int_feat)
        ...
        fully1 = self.selu(self.fc1(cat_v))
        fully1 = self.dropout(fully1)
        fully2 = self.selu(self.fc2(fully1))
        fully2 = self.dropout(fully2)
        fully3 = self.selu(self.fc3(fully2))
